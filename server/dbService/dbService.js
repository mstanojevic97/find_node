...
async function updateCompany(idCompany,companyName,VAT,email,password)
{
    let sql="UPDATE company SET companyName=?,VAT=?,email=?,password=? WHERE idCompany=?"
    return request.query(sql,[companyName,VAT,email,password,idCompany]);
}
